...
OK相信有了上面的例子，已经对unittest 有了一个初步的印象。
...
OK可以看到，unpack 对每次传入的包进行解包，例如将 ('苹果', '香蕉') 解包，'苹果' 给t
...
OK上面的方法每次添加测试用例都需要调用 addTest 函数，能不能一次添加多个测试用例
12.py
#coding=utf-8
import unittest
from testCase import *
...
下载后，把HTMLTestRunner.py 文件复制到 Python 安装路径下的 lib 文件夹中。在python3中用HTMLTestRunner.py会报错，原因是HTMLTestRunner仅支持python2，不支持python3，需要做小小的改动。
修改内容：
